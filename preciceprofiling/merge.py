...
def warning(message: str, filename=None) -> None:
...
def groupEvents(
    cur: sqlite3.Cursor, pid: int, rank: int, events: List[Dict], initTime: int
):
...
def runMerge(ns: argparse.Namespace) -> Literal[0]:
    return mergeCommand(ns.files, ns.output, not ns.no_align)


def mergeCommand(files, outfile, align: bool) -> Literal[0]:
...
def makeMergeParser(add_help: bool = True) -> argparse.ArgumentParser:
